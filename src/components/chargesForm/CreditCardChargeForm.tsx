...
const credValidationSchema = Yup.object().shape({
...
const CreditCardChargeForm: React.FC<CreditCardChargeFormProps> = ({ addToast }) => {
  const handleSubmit = async (values: any, { resetForm }: any) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        addToast("Cobrança no crédito criada com sucesso!", "success");
        resetForm();
      } else {
